...
def blur(img):
    ...
    """
    Solution 1: Opt to use 4 for-loops
    """
    # Loop over the picture
    for x in range(img.width):
        for y in range(img.height):
            r_sum = 0  # The sum of red bytes from designated pixels
            g_sum = 0  # The sum of green bytes from designated pixels
            b_sum = 0  # The sum of blue bytes from designated pixels
            count = 0
                ...
                for j in range(-1, 2, 1):
                    ...
                    if 0 <= pixel_x < img.width:
                        if 0 <= pixel_y < img.height:
                            img_pixel = img.get_pixel(pixel_x, pixel_y)
                            r_sum += img_pixel.red
                            g_sum += img_pixel.green
    ...
    """
    Solution 2: Divide into 9 conditions of pixel filling, resulting in long redundant codes
    """
    # # Loop over the picture
    # for x in range(img.width):
    #     for y in range(img.height):
